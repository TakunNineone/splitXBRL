...
class splitXBRL():
    ...
    def readXBRL(self,path):
        ...
        varibles=[varible for varible in instance['xbrli:xbrl'].keys() if '@' not in varible and varible not in ('link:schemaRef','xbrli:context','xbrli:unit')]
        #print(instance['xbrli:xbrl']['bki-dic:DataVstupVSiluDogObUstupPravTreb'])

        for idx,context in enumerate(instance['xbrli:xbrl']['xbrli:context']):
        ...
        with open('XBRL_1047820008895_ep_nso_bki_q_y_15rd_20230331-split.xml','w',encoding='utf-8') as f:
            f.write(xml)


if __name__ == "__main__":
    ss=splitXBRL()
    ss.readXBRL('XBRL_1047820008895_ep_nso_bki_q_y_15rd_20230331.xml')
